...
const projectile_path = "res://bean/pistol/bullet/bullet.tscn"
...
func _process(delta):
		...
		if global_position > get_global_mouse_position():
			flip_v = 1
			get_parent().get_node("sprite").flip_h = 1
		else:
			flip_v = 0
			get_parent().get_node("sprite").flip_h = 0

func shoot():	
	var bullet = preload(projectile_path).instance()
