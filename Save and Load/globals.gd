...
func save_data():
	...
	var err = save_file.open_encrypted_with_pass(SAVFILE, File.WRITE, IP.get_local_addresses()[0])
...
#	save_file.open(SAVFILE, File.READ)
	save_file.open_encrypted_with_pass(SAVFILE, File.READ, IP.get_local_addresses()[0])
